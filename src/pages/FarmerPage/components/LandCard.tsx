...
export default function LandCard(props: LandCardProps) {
  ...
  const [decodedLocation, loadingDecodedLocation] = useFetch<{
    ...
    `http://api.positionstack.com/v1/reverse?access_key=${"f66d8b1d1213aaa04b49b2fc4d06c340-+"
    ...
    <div className="flex rounded-2xl border border-front border-opacity-30 shadow-md">
      ...
      <div className="flex flex-1 flex-col gap-y-5 p-6 text-lg">
        ...
        <p className="text-xl">
          Location :{" "}
          <span
            className={`${loadingDecodedLocation
      ...
      <div className="m-5 h-full w-[18%] self-center">
        <button
          ...
          className={`group flex aspect-square w-full flex-col items-center justify-center gap-y-5 rounded-xl bg-opacity-10 text-2xl duration-300 hover:bg-opacity-100 hover:text-background ${!land.cropId
            ? "bg-primary text-primary"
            : land.insurance && land.insurance.isInsured
              ? "bg-red-500 text-red-500"
